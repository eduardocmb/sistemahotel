export class search {
    constructor(myurlp, mysearchp, ul_add_lip) {
        ...
        this.pcodigo = document.getElementById('codigo');
    ...
    InputSearch() {
        ...
        // Evento para capturar la tecla Enter o navegación con flechas
        this.mysearch.addEventListener("keydown", (e) => {
            ...
            } else if (e.key === "Enter") { // Cambiado de Enter a F1
                e.preventDefault(); // Impide la acción predeterminada
                if (selected) {
                    selected.click();
                }
            }
        ...
        for (let item of arrayp) {
            ...
            li.innerHTML = `
                <div class="d-flex flex-row">
                    <div class="p-2">
                       <span>${item.codigo} - </span> <strong data-presentacionporpum="${item.contiene}" data-idprod="${item.id}" data-prod="${item.nombre}" data-isv="${item.isv}" data-presentacion="${item.unidad_nombre}/${item.contiene}" data-precio="${item.precio_venta}" data-codigo = "${item.codigo}"> ${nombre.substr(0, valor.length)}</strong>${nombre.substr(valor.length)}
