...
class CabfacturainternoController extends Controller
{
    /**
    ...
    public function index()
    {
        ...
        return view('facturacion.facturacionInterno.create');
    }
    ...
    public function store(Request $request)
    {
        ...
        $cabfactura = new cabfacturainterno();
        ...
        foreach ($productos as $index => $producto) {
            $detalle = new detallefacturainterno();
        ...
        if ($imprimir_flag) {
            return redirect()->route('facturasin.rptfacturaIn', $cabfactura->id)->with('etiqueta', 'ORIGINAL');
        } else {
     ...
     */
    public function show(cabfacturainterno $cabfacturainterno)
    {
        //
    }

    public function anularInterno($id_factura){
        if ($this->rol->eliminar == "N") {
            return redirect()->route('dashboard')->with('nopermiso', 'No tienes permiso para acceder a la información solicitada.');
        }
        $factura = cabfacturainterno::where('id', $id_factura)->first();
        $factura->anular = 'S';
        $factura->save();
        $kardex = new KardexController();

        $prods = detallefacturainterno::where('cabfacturainterno_id', $id_factura)->get();
        foreach($prods as $prod){
            ...
            if($producto->tipo_producto === "PRODUCTO FINAL"){
                ...
                $kardex->setKardex($id_factura, 'ENTRADA', 'ANULACION DE FACTURA ID:' . $id_factura, $codigo, $lote->cantidad, (intval($contiene)*intval($prod->cant)), 0, intval($lote->cantidad) + (intval($contiene)*intval($prod->cant)));
     ...
     */
    public function edit($cabfacturainterno_id)
    {
        $factura = cabfacturainterno::select(
            'cabfacturainternos.*',
            'cabfacturainternos.id as cab_id',
            ...
            ->where('cabfacturainternos.id', $cabfacturainterno_id)
            ->get();
        return view('facturacion.facturacionInterno.edit', compact('factura'));
    }

    /**
     * Update the specified resource in storage.
     */
    public function update(Request $request, cabfacturainterno $cabfacturainterno)
    {
        //
    }
     ...
     */
    public function destroy(cabfacturainterno $cabfacturainterno)
    {
        //
    }
}
